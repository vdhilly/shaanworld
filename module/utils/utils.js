...
export function getSelectedActors(types) {
  const actors = canvas.tokens.controlled
    .flatMap((token) => (token.actor ? token.actor : []))
    .filter((actor) => types.includes(actor.type));
...
export function htmlQuery(parent, selectors) {
  return parent instanceof Element || parent instanceof Document
...
export function fontAwesomeIcon(
  ...
  return (
    ...
    fixedWidth && icon.classList.add("fa-fw"),
    icon
  );
}
...
export function isItemSystemData(data) {
  return (
    (0, isObject)(data) &&
    (0, isObject)(data.description) &&
    "string" == typeof data.description.value &&
    Array.isArray(data.rules) &&
    (null === data.slug || "string" == typeof data.slug)
  );
}
